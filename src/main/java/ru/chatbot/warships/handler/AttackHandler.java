...
public class AttackHandler implements Handler {
    private Pattern tradePattern = Pattern.compile("\\/attack_(\\d)+");
    ...
    public SendMessage handle(Update update) {
        ...
        if (port.getOwner().equals(player.getTeam())) {
            return Message.makeReplyMessage(update, Message.getAttackOwnPort(), keyboardMarkup);
        }

        try {
            voyageService.createAttack(player, player, playerService.getPlayerLocation(player.getId()), destinationId);
            return Message.makeReplyMessage(update, Message.getAttackMessage(), keyboardMarkup);
